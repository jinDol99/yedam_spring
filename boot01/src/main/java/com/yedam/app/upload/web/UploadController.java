...
public class UploadController {
	...
	// void: 경로를 기반으로 파일을 생선한 경우 사용
	// classpath: /templates/ + URI + .html
	@GetMapping("formUpload")
	public void formUploadPath() {}
	...
	public String formUploadFile(@RequestPart MultipartFile file) {
		...
		String saveName = uploadPath + File.separator + fileName;	// 파일 경로
...
/*
	[7-3] 첨부파일을 여러개 선택하여 업로드할 수 있음.
		단순 업로드 뿐만 아니라 저장하는 디렉터리를 날짜별로 자동 분류,
		중복된 파일일 경우엔 UUID를 달리 설정하여 업로드 가능케끔 설정,
		이미지 파일 외 업로드 시도할 경우 막기와 같은 기능도 같이 사용함.
		
		직접 타이핑하긴 복잡하니 가져온 코드를 참고할 것.
